export class DataWorkerClient {
  constructor(worker = new Worker(new URL('./data.worker.js', import.meta.url), { type: 'module' })) {
    this.worker = worker;
    this.nextId = 0;
    ...
    worker.onmessage = ({ data }) => {
      ...
      if (data.error) {
        const error = new Error(data.error.message || 'Unknown worker error');
        error.name = data.error.name || 'Error';
        if (data.error.stack) error.stack = data.error.stack;
        if (data.error.cause) error.cause = data.error.cause;
